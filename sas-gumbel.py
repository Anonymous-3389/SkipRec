...
from models.sasrec_gumbel import (
    SASRec,
    SASPolicyNet,
)
...
def start(global_args):
    ...
    with tf.variable_scope("policy_net"):
        policy_net = SASPolicyNet(model_args)
        # policy_net = PolicyNetGumbelRandom(model_args)

    with tf.variable_scope(tf.get_variable_scope()):
        model = SASRec(model_args)
        model.build_train_graph(policy_action=policy_net.action_predict_train)
        model.build_test_graph(policy_action=policy_net.action_predict_test)

    # l2_reg_policy = tf.contrib.layers.l2_regularizer(0.02)
    # with tf.variable_scope("policy_net", regularizer=l2_reg_policy):
    #     policy_net = SASPolicyNet(model_args)
    #     reg_losses_policy = tf.reduce_mean(
    #         tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
    #     )
    #     # policy_net = PolicyNetGumbelRandom(model_args)
    #
    # l2_reg_model = tf.contrib.layers.l2_regularizer(0.02)
    # with tf.variable_scope(tf.get_variable_scope(), regularizer=l2_reg_model):
    #     model = SASRec(model_args)
    #     model.build_train_graph(policy_action=policy_net.action_predict_train)
    #     model.build_test_graph(policy_action=policy_net.action_predict_test)
    #     reg_losses_model = tf.reduce_mean(
    #         tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
    #     )
    ...
    with tf.variable_scope(tf.get_variable_scope()):
        ...
        # loss_model = model.loss_train + reg_losses_model
        # train_model = optimizer_finetune.minimize(loss_model, var_list=model_variables)
    ...
    with tf.variable_scope("policy_net"):
        ...
        # loss_policy = model.loss_train + reg_losses_policy
        # train_policy = optimizer_policy.minimize(loss_policy, var_list=policy_variables)
    ...
    if global_args["use_pre"]:
        ...
        # step-4.2 copy embeddings to policy-net
    ...
    for idx in range(total_iters):
        ...
        for batch_step in range(total_steps):
            ...
            _, _, loss, action = sess.run(
                [
                    ...
                    train_policy,
                    model.loss_train,
                    policy_net.action_predict_train,
                ],
                feed_dict={
            ...
            if num_iter % log_meter == 0:
                ...
                # logging.info("\t{}".format(action[:10]))
            num_iter += 1

        summary_block(train_usage_sample, model_args["num_blocks"], "Train")
        ...
        for batch_step in range(test_steps):
            ...
            action, pred_probs = sess.run(
                [policy_net.action_predict_test, model.probs_test],
        ...
        summary_block(test_usage_sample, model_args["num_blocks"], "Test")
