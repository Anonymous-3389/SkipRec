...
from models.nextitnet_gumbel_gru_rl import NextItNetGumbelRL, PolicyNetGumbelGru
...
def start(global_args):
    ...
    for idx in range(start_at, total_iters):
        ...
        if idx < global_args["rl_iter"]:
            ...
            rl_str = " ON"
        logging.info("[RL-{}] Iter: {} / {}".format(rl_str, idx + 1, total_iters))
        num_iter = 1
        tic = time.time()
        ...
        for batch_step in range(total_steps):
            ...
            if idx >= global_args["rl_iter"]:
                ...
                [_, _, action, loss] = sess.run(
                    [train_model, train_policy, policy_net.action_predict, model.loss],
                    feed_dict={
                        model.input_train: train_batch,
                        policy_net.input: train_batch,
...
def parse_arg(args_inline=None, **patch):
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Start RL training phase after {x} iterations.",
    )
    parser.add_argument(
